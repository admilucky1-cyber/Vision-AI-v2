...
GUEST_MAX = int(os.getenv("GUEST_MESSAGES_MAX", "1"))
FREE_MAX = int(os.getenv("FREE_MESSAGES_PER_MONTH", "10"))
# Optional daily cap for free (same as month if not set lower)
FREE_DAILY = int(os.getenv("FREE_MESSAGES_PER_DAY", "10"))
    ...
    with _lock:
        ...
        if is_guest:
            if int(bucket.get("count") or 0) >= max(0, GUEST_MAX):
                return (
                    False,
                    "Guest limit reached (1 free reply). Please sign in to continue. Free accounts get 10 messages, then upgrade for unlimited-style paid plans.",
                    meta,
                )
            return True, "ok", meta
        if int(bucket.get("month_count") or 0) >= max(0, FREE_MAX) or int(
            ...
            return (
                False,
                f"Free plan limit reached ({FREE_MAX} messages). Upgrade to Student or Pro for more capacity — open Plans /upgrade.html.",
                meta,
            )
        return True, "ok", meta
