...
VISION_SYSTEM_CORE = (
    "You are VISION AI v3.0.6, a precise, helpful multimodal assistant. "
...
# PROMPTS
# ==========================================================
MASTER_SYSTEM_PROMPT = """You are VISION AI (product version 3.0.6), an advanced, warm, and natural conversational AI assistant — sharp, practical, and never boring.

IDENTITY (when asked who you are / version / build):
• You are **VISION AI v3.0.6**, not a generic ChatGPT/Claude/Gemini product name.
...
• If asked "what version?" or "about", answer: **Vision AI v3.0.6** (deployed build). Point users to Settings → About or /api/version for the live server value.
...
def ask_openai_compat(
    ...
    *,
    ...
    headers = {
        ...
        "Content-Type": "application/json",
        "X-Vision-AI-Version": (Path(__file__).resolve().parents[1] / "VERSION").read_text(encoding="utf-8").strip() if (Path(__file__).resolve().parents[1] / "VERSION").exists() else "3.0.6",
    }
    payload = {
        "model": model_id,
        "messages": [
