...
TIMEOUT_IMAGE = float(os.getenv("COLAB_IMAGE_TIMEOUT", "180"))
...
def heartbeat_worker(url: str, secret: str = "") -> dict:
    ...
    with _LOCK:
        ...
        _save(rows)
    return {"ok": True, "url": url, "created": True}
...
def chat(question: str, context: str = "", model: str = "auto") -> Optional[str]:
    ...
    for u in order:
        try:
            ...
            if len(answer) > 10:
                logger.info(f"Worker chat OK via {u}")
                return answer
        except Exception as e:
            logger.warning(f"worker chat {u}: {e}")
    return None
