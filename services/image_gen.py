...
async def generate_creative_image(prompt: str) -> Dict[str, Any]:
    """Generate a real image via HF (FLUX/SD) with a creative prompt — not ASCII."""
    if not HF_TOKEN:
        logger.error("HF_TOKEN not set — cannot generate creative images")
        return {
            "success": False,
            "error": "HF_TOKEN not set in server .env. Add a free Hugging Face token to enable real image generation.",
        }
