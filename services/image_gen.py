...
def _is_educational_anatomy(text: str) -> bool:
    t = (text or "").lower()
    keys = (
        "anatomy", "anatomical", "medical student", "organ", "physiology",
        "diagram of", "labeled", "labelled", "textbook", "clinical",
...
def clean_image_prompt(raw: str) -> str:
    ...
    for prefix in (
        ...
        if low.startswith(prefix):
            ...
            low = p.lower()
    # Remove "which demonstrates/shows/represents"
    p = re.sub(
    ...
    p = re.sub(r"\bhigh\s*res(olution)?\b", "", p, flags=re.I).strip()
    p = re.sub(r"\s{2,}", " ", p).strip(" ,.-")
    if len(p) < 3:
        p = (raw or "illustration").strip()
    # Strict composition hint (does not replace subject)
    if _is_educational_anatomy(raw) or _is_educational_anatomy(p):
        return (
            f"Educational medical anatomy textbook illustration of {p}, "
            f"clinical diagram style, clear labels, white background, "
            f"professional medical education, not sexualized, not pornographic"
        )
...
async def generate_creative_image(prompt: str) -> Dict[str, Any]:
    """Generate a real image via Colab worker (preferred) or HF — strict user prompt."""
    strict = clean_image_prompt(prompt)
    logger.info(f"Creative image strict prompt: {strict[:120]}")

    # Prefer Colab GPU worker when live
    try:
        from services.colab_worker import is_enabled as _colab_on, generate_image as _colab_img
        if _colab_on():
            got = _colab_img(strict)
            if got and got.get("success") and got.get("image_data"):
                return got
    except Exception as e:
        logger.warning(f"Colab image skip: {e}")

    # Optional RunPod serverless GPU (user's own key/endpoint)
    try:
        from services.runpod_worker import is_enabled as _rp_on, generate_image as _rp_img
        if _rp_on():
            got = await asyncio.to_thread(_rp_img, strict)
            if got and got.get("success") and got.get("image_data"):
                return got
    except Exception as e:
        logger.warning(f"RunPod image skip: {e}")
    ...
    if not HF_TOKEN:
        try:
            ...
            logger.warning(f"Pollinations skip: {e}")
        logger.error("HF_TOKEN not set — cannot generate creative images")
        return {
            "success": False,
            "error": "No image provider available. Set HF_TOKEN, start Colab Boost, or configure RunPod.",
        }
    return await generate_with_hf_creative(strict)
