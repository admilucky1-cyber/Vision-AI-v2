...
def user_wants_creative_image(user_message: str) -> bool:
    ...
    charty = (
        "chart", "graph", "plot", "flowchart", "org chart",
        "diagram", "pie chart", "bar chart", "force diagram", "speed-time",
    )
    if any(c in m for c in charty):
        return False
    return any(c in m for c in creative) or (
        ("image" in m or "picture" in m or "photo" in m) and any(
...
async def generate_all_diagrams(
    ...
    images = []
    intent = (user_message or answer_text or "")[:300]

    # Creative / photo-style request → HF FLUX (not ASCII, not educational diagram prompt)
    if user_wants_creative_image(user_message):
        ...
        # fall through to diagram types / HF diagram path
    ...
    # If user asked for an image but no chart type matched, still try HF with free prompt
    if not diagram_types and user_wants_diagram(user_message):
        logger.info("No chart type — trying HF with free prompt")
        result = await generate_creative_image(intent)
        if result.get("success"):
            result["keyword"] = "freeform"
            result["diagram_type"] = "creative"
            images.append(result)
        return images
...
async def generate_creative_image(prompt: str) -> Dict[str, Any]:
    """Generate a real image via Colab worker (preferred) or HF (FLUX/SD) — not ASCII."""
    # Prefer Colab GPU worker when live
    try:
        from services.colab_worker import is_enabled as _colab_on, generate_image as _colab_img
        if _colab_on():
            got = _colab_img(prompt)
            if got and got.get("success") and got.get("image_data"):
                return got
    except Exception as e:
        logger.warning(f"Colab image skip: {e}")

    if not HF_TOKEN:
        logger.error("HF_TOKEN not set — cannot generate creative images")
        return {
            "success": False,
            "error": "HF_TOKEN not set and Colab worker offline. Set HF_TOKEN or start Colab worker (COLAB_WORKER_URL).",
        }
    # Clean prompt: strip "create a" noise, keep subject
    p = (prompt or "").strip()
    for noise in (
        "create a image", "create an image", "create a picture", "create an picture",
        "generate an image", "generate a image", "generate a picture",
        "make an image", "make a picture", "draw me", "please",
    ):
        p = p.lower().replace(noise, " ").strip() if p.lower().startswith(noise[:8]) else p
    enhanced = (
        f"{prompt}, highly detailed, sharp focus, natural lighting, "
        f"professional photography quality, 4k"
    )
    # Bypass educational-diagram wrapper used by generate_with_hf
    return await generate_with_hf_creative(enhanced)
