...
async def download_video(
    ...
    *,
    ...
    def _build_args(
        ...
        try:
            from services.ytdlp_proxy import get_working_proxy, invalidate_proxy
            proxy = get_working_proxy()
        except Exception:
            proxy = (os.getenv("YTDLP_PROXY") or os.getenv("HTTPS_PROXY") or os.getenv("HTTP_PROXY") or "").strip() or None
        if proxy:
    ...
    strategies = [
        {"player_client": "android", "format_override": None, "cookies": False},
        {"player_client": "ios", "format_override": None, "cookies": False},
        {"player_client": "mweb", "format_override": None, "cookies": False},
        {"player_client": "tv_embedded", "format_override": None, "cookies": False},
        {"player_client": "web", "format_override": None, "cookies": False},
        {
        ...
        # Cookie-assisted last (if admin provided Netscape cookies.txt)
        {"player_client": "android", "format_override": progressive_v if not audio_only else progressive_a, "cookies": True},
        {"player_client": "web", "format_override": progressive_v if not audio_only else progressive_a, "cookies": True},
    ]
    result = None
    last_err = ""
    try:
        for i, strat in enumerate(strategies):
            ...
            logger.info(
                f"yt-dlp attempt {i+1}/{len(strategies)} client={strat['player_client']} cookies={use_c}"
            )
    ...
    if result is None or result.returncode != 0:
        ...
        if _is_bot_block(err):
            err_short = (
                "YouTube blocked this server IP (bot check). "
                "Fixes: (1) Export cookies.txt from your browser (extension: 'Get cookies.txt LOCALLY'), "
                "place it in the project root, set Railway env YTDLP_COOKIES=cookies.txt, redeploy. "
                "(2) Optional: YTDLP_PROXY=http://user:pass@host:port. "
                "(3) Retry in a few minutes or use a public/unlisted video. "
                "Docker image must include ffmpeg + latest yt-dlp."
            )
