    ...
    *,
    ...
    def _build_args(use_cookies: bool = False, use_browser: bool = False) -> list:
        a = [
            ...
            "--retries", "5",
            "--fragment-retries", "5",
            # android client often avoids bot checks without cookies
            "--extractor-args", "youtube:player_client=android,ios,web,tv_embedded",
            "--user-agent",
    ...
    if result.returncode != 0:
        ...
        if _is_cookie_db_error(err):
            err_short = (
                "Cookie database error (browser profile locked or missing). "
                "Public downloads do not need browser cookies. "
                "If YouTube blocks the request, export a cookies.txt and set YTDLP_COOKIES=/path/to/cookies.txt."
            )
