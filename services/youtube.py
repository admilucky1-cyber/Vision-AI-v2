...
async def download_video(
    ...
    *,
    ...
    # Multi-strategy: public videos usually work with android client, no cookies
    strategies = [
        {"player_client": "android", "format_override": None},
        {"player_client": "ios", "format_override": None},
        {"player_client": "mweb", "format_override": None},
        {"player_client": "tv_embedded", "format_override": None},
        # Progressive single-file fallback (no merge needed)
        {
            "player_client": "android",
            "format_override": (
                "best[height<=720][ext=mp4]/best[height<=480]/best"
                if not audio_only
    ...
    try:
        for i, strat in enumerate(strategies):
            ...
            logger.info(f"yt-dlp attempt {i+1}/{len(strategies)} client={strat['player_client']}")
            result = await asyncio.to_thread(_run, args)
            if result.returncode == 0:
                break
            last_err = (result.stderr or result.stdout or "").strip()
            # Skip cookie-db noise from any env leakage
            if _is_cookie_db_error(last_err):
                logger.warning("Ignoring cookie-database noise; trying next client")
                continue
            if not _is_bot_block(last_err) and "requested format is not available" not in last_err.lower():
                # Hard failure (private video, etc.) — still try other clients once more
                continue
    except FileNotFoundError:
    ...
    # Optional cookies file only if still failing
    if result is not None and result.returncode != 0 and _resolve_cookies_file():
        logger.info("Retrying with cookies file")
        try:
            result = await asyncio.to_thread(
                _run, _build_args(use_cookies=True, use_browser=False, player_client="android")
            )
        except Exception as e:
            return {"status": "error", "error": str(e)}
    ...
    if result is None or result.returncode != 0:
        ...
        if _is_bot_block(err):
            err_short = (
                ...
                "Use a public video, wait a few minutes, or set YTDLP_COOKIES to a Netscape cookies.txt export."
            )
