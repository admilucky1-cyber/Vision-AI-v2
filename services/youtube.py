...
async def download_video(
    ...
    *,
    ...
    outtmpl = str(DOWNLOAD_DIR / f"media_{timestamp}_{video_id}.%(ext)s")
    ...
    def _build_args(use_cookies: bool = False, use_browser: bool = False) -> list:
        # --no-config: ignore system/user yt-dlp.conf that may force --cookies-from-browser
        # (that causes "Cookie database error" on Railway/Docker with no Chrome profile)
        a = [
            *_ytdlp_cmd(),
            "--no-config",
            "-f", fmt,
            ...
            "--retries", "8",
            "--fragment-retries", "8",
            # Prefer clients that work without browser cookies on servers
        ...
        # Browser cookies ONLY when explicitly opted in (local desktop). Never on Railway by default.
        if use_browser:
    ...
    def _is_cookie_db_error(err: str) -> bool:
        e = (err or "").lower()
        return "cookie database" in e or "cookies-from-browser" in e

    # Attempt 1: no cookies (works for most public videos, avoids Chrome DB errors)
    try:
        result = await asyncio.to_thread(_run, _build_args(use_cookies=False, use_browser=False))
    except FileNotFoundError:
    ...
    # Attempt 2: retry with cookies file if bot-blocked and cookies exist
    if result.returncode != 0:
        err1 = (result.stderr or result.stdout or "").strip()
        if _is_bot_block(err1) and _resolve_cookies_file():
            logger.info("Bot check detected — retrying with cookies file")
            try:
                result = await asyncio.to_thread(_run, _build_args(use_cookies=True, use_browser=False))
            except Exception as e:
                return {"status": "error", "error": str(e)}
        elif _is_bot_block(err1) and _browser_cookies_allowed():
            logger.info("Bot check detected — retrying with browser cookies (opt-in)")
            try:
                result = await asyncio.to_thread(_run, _build_args(use_cookies=False, use_browser=True))
            except Exception as e:
                return {"status": "error", "error": str(e)}
    ...
    if result.returncode != 0:
        ...
        if _is_cookie_db_error(err):
            err_short = (
                "YouTube download failed on the server (no browser cookies available). "
                "Try again in a minute, or use a public video URL. "
                "Admin: place cookies.txt in the project and set YTDLP_COOKIES, and ensure ffmpeg is in the Docker image."
            )
        elif _is_bot_block(err):
            err_short = (
                "YouTube blocked the download (bot check). "
                "Export cookies from your browser (extension: 'Get cookies.txt LOCALLY') "
                "and set YTDLP_COOKIES to that file path, or place cookies.txt in the project root."
            )
        elif "ffprobe" in err.lower() or "ffmpeg" in err.lower() or "postprocessing" in err.lower():
            err_short = (
                "Audio/video conversion needs ffmpeg, which is not installed on this server. "
                "Install it with: `sudo apt install -y ffmpeg`  "
                "(or deploy using the project Dockerfile, which includes ffmpeg). "
                "You can also set FFMPEG_LOCATION to the binary path."
            )
        else:
            # Strip noisy multi-line yt-dlp noise; keep last meaningful line
    ...
    matches = sorted(
        DOWNLOAD_DIR.glob(f"media_{timestamp}_{video_id}.*"),
        key=lambda p: p.stat().st_mtime,
        reverse=True,
    )
    if not matches:
        matches = sorted(
            DOWNLOAD_DIR.glob(f"media_{timestamp}*"),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        )
    if not matches:
