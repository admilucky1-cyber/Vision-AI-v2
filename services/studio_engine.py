...
async def generate_image(
    *,
    ...
    # Propagate model/lora into worker payload
    worker_payload = {
        "prompt": prompt,
        ...
        "lora_id": lora_id,
        "lora_path": (lora or {}).get("drive_path") or (lora or {}).get("path"),
        "lora_weight": lora_weight,
        "width": width,
        "height": height,
        "steps": steps,
    ...
    try:
        ...
        if hasattr(cw, "is_live") and cw.is_live():
            ...
            if gen:
                try:
                    result = gen(prompt, width=width, height=height, **{k: v for k, v in worker_payload.items() if k not in ("prompt", "width", "height")})
                except TypeError:
                    result = gen(prompt, width=width, height=height)
