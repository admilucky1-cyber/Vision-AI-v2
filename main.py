"""
Vision AI v3.2.2 - Production-Grade Multi-Modal AI Assistant
============================================================
...
APP_VERSION = (BASE_DIR / "VERSION").read_text(encoding="utf-8").strip() if (BASE_DIR / "VERSION").exists() else "3.2.2"
...
class AppConfig:
    ...
    def validate(cls) -> List[str]:
        ...
        if not cls.DEBUG and (not cls.ALLOWED_HOSTS or cls.ALLOWED_HOSTS == ["*"]):
            errors.append("ALLOWED_HOSTS must be explicitly configured in production (do not use *).")
        if errors:
            for e in errors:
                logger.error(e)
...
async def api_version():
    ...
    return {
        ...
        "server_time_pkt": utc_now.astimezone(pkt).strftime("%Y-%m-%d %H:%M:%S PKT"),
        "build": ver,
    }
