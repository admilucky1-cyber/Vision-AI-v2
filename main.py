...
class AppConfig:
    ...
    # Rate Limiting
    RATE_LIMIT_SEARCH: str = os.getenv("RATE_LIMIT_SEARCH", "10/minute")
    RATE_LIMIT_DOWNLOAD: str = os.getenv("RATE_LIMIT_DOWNLOAD", "5/minute")
...
async def lifespan(app: FastAPI):
    ...
    # Create directories
    for dir_path in [AppConfig.UPLOAD_DIR, AppConfig.CACHE_DIR, AppConfig.LOG_DIR, AppConfig.DOWNLOAD_DIR]:
    ...
    if request.method == "POST" and request.headers.get("content-type"):
        ...
        if not any(vt in content_type for vt in valid_types):
            return JSONResponse(
                ...
                content={"error": "Invalid content type", "detail": f"Received: {content_type}"}
            )
...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"] if AppConfig.DEBUG else AppConfig.ALLOWED_HOSTS,
...
# ==========================================================
# STATIC FILES & FRONTEND ROUTING (💎 THE FIX)
# ==========================================================
...
# 3. 🟢 FIX: Explicitly serve upgrade.html
...
# 4. 🟢 FIX: Explicitly serve settings.html
...
# 5. 🟢 FIX: Explicitly serve login.html
@app.get("/login.html")
