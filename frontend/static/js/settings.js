...
function renderApiKeysCard(container) {
    ...
    card.innerHTML = `
        ...
        <div class="field"><label>Google / Gemini</label><input id="key_gemini" type="password" placeholder="AIza..." value="${saved.GOOGLE_API_KEY || ''}"></div>
        <div class="field"><label>Groq</label><input id="key_groq" type="password" placeholder="gsk_..." value="${saved.GROQ_API_KEY || ''}"></div>
        <div class="field"><label>DeepSeek</label><input id="key_deepseek" type="password" placeholder="sk-..." value="${saved.DEEPSEEK_API_KEY || ''}"></div>
        <div class="field"><label>OpenRouter</label><input id="key_openrouter" type="password" placeholder="sk-or-..." value="${saved.OPENROUTER_API_KEY || ''}"></div>
        <div class="field"><label>Tavily Search</label><input id="key_tavily" type="password" placeholder="tvly-..." value="${saved.TAVILY_API_KEY || ''}"></div>
        <label style="display:flex;align-items:center;gap:8px;margin:12px 0;font-size:13px;">
            <input type="checkbox" id="key_override" ${saved.override ? 'checked' : ''}> Prefer these keys over server defaults (advanced)
        </label>
    ...
    document.getElementById('saveKeysBtn').onclick = () => {
        ...
        document.getElementById('keysStatus').textContent = '✅ Saved in this browser. Restart is not required for chat client; server still uses .env unless admin endpoint is configured.';
    };
    document.getElementById('clearKeysBtn').onclick = () => {
        localStorage.removeItem('vision_ai_user_keys');
        ['key_gemini','key_groq','key_deepseek','key_openrouter','key_tavily'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; });
...
(function loadAbout(){
  function run(){
    ...
    fetch('/api/version').then(r=>r.json()).then(d=>{
      if (v) v.textContent = (d.version || d.current || '2.4.x');
    }).catch(()=>{ if (v) v.textContent = '2.4.9'; });
  }
