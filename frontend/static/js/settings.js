// ============================================================
// VISION AI v3.1.1 - SETTINGS LOGIC (Production Ready)
// ============================================================
        ...
        if (content) {
            content.innerHTML = `
                <div class="loading-state" style="text-align:center;padding:40px;color:var(--text-muted);">
                    ⚠️ ${escapeHtml(err.message || 'Failed to load settings')}
                    <br><br>
                    <button onclick="window.location.reload()" class="btn btn-primary">Retry</button>
                </div>
            
        <div class="card full-width" id="aboutCard">
            <h2>ℹ️ About Vision AI</h2>
            <div class="profile-row"><span class="label">App</span><span class="value">Vision AI Regenerative</span></div>
            <div class="profile-row"><span class="label">Version</span><span class="value" id="aboutVersion">…</span></div>
            <div class="profile-row"><span class="label">Loaded</span><span class="value" id="aboutTime"></span></div>
            <p style="font-size:13px;color:var(--text-muted);margin:12px 0 0;line-height:1.5;">
              Free-first chat (Groq / Gemini / OpenRouter). Optional GPU Boost (Colab / Kaggle).
              Manual PKR payments. Educational diagrams and medical study content supported.
            </p>
            <div class="card-actions" style="margin-top:12px;">
              <button type="button" class="btn btn-outline" onclick="window.location.href='/versions.html'">Versions</button>
              <button type="button" class="btn btn-outline" onclick="window.location.href='/boost'">GPU Boost</button>
              <button type="button" class="btn btn-outline" onclick="window.location.href='/upgrade.html'">Plans</button>
            </div>
        </div>
`;
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('👁️ Vision AI Settings v3.1.1 - Ready');
});
...
function renderApiKeysCard(container) {
    ...
    card.innerHTML = `
        ...
        <div class="field"><label>Google / Gemini</label><input id="key_gemini" type="password" placeholder="AIza..." value="${saved.GOOGLE_API_KEY || ''}" autocomplete="off"></div>
        <div class="field"><label>Groq</label><input id="key_groq" type="password" placeholder="gsk_..." value="${saved.GROQ_API_KEY || ''}" autocomplete="off"></div>
        <div class="field"><label>DeepSeek</label><input id="key_deepseek" type="password" placeholder="sk-..." value="${saved.DEEPSEEK_API_KEY || ''}" autocomplete="off"></div>
        <div class="field"><label>OpenRouter</label><input id="key_openrouter" type="password" placeholder="sk-or-..." value="${saved.OPENROUTER_API_KEY || ''}" autocomplete="off"></div>
        <div class="field"><label>Tavily Search</label><input id="key_tavily" type="password" placeholder="tvly-..." value="${saved.TAVILY_API_KEY || ''}" autocomplete="off"></div>
        ...
        <div class="field"><label>Base URL</label><input id="key_compat_base" type="text" placeholder="http://127.0.0.1:11434 or http://127.0.0.1:1234/v1" value="${saved.OPENAI_COMPAT_BASE || ''}"></div>
        <div class="field"><label>API key (optional)</label><input id="key_compat_key" type="password" placeholder="ollama / lm-studio / sk-..." value="${saved.OPENAI_COMPAT_KEY || ''}" autocomplete="off"></div>
        <div class="field"><label>Model id</label><input id="key_compat_model" type="text" placeholder="llama3.2 / local-model / qwen2.5" value="${saved.OPENAI_COMPAT_MODEL || ''}"></div>
        <label style="display:flex;align-items:center;gap:8px;margin:12px 0;font-size:13px;">
            <input type="checkbox" id="key_override" ${saved.override ? 'checked' : ''}> Prefer these keys over server defaults (sends on every chat request)
        </label>
...
(function loadAbout(){
  function run(){
    ...
    fetch('/api/version').then(r => r.json()).then(d => {
      if (v) v.textContent = d.version || d.current || '3.1.1';
    ...
    }).catch(() => { if (v) v.textContent = '3.1.1 (offline meta)'; });
...
document.addEventListener("DOMContentLoaded", function() {
  var sel = document.querySelector("select[id*='theme'], select.theme-preset, #themePreset");
  if (!sel) return;
  try {
  ...
  sel.addEventListener("change", function() {
    var v = sel.value;
    try { localStorage.setItem("vision_theme_preset", v); } catch (e) {}
    document.documentElement.setAttribute("data-theme-preset", v === "default" ? "" : v);
    if (v === "sand" || v === "soft-sepia") {
      document.documentElement.setAttribute("data-theme", "light");
    } else {
      document.documentElement.setAttribute("data-theme", "dark");
    }
