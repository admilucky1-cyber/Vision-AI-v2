...
function applyTheme(theme) {
    ...
    // Skip DOM writes if unchanged — avoids full-style recalculation flash
...
async function sendMessage() {
    ...
    try {
        ...
        removeLoading();
        if (res.status === 405) {
            if (typeof addMessage === 'function') addMessage('ai', '❌ Chat endpoint returned 405 Method Not Allowed. Please redeploy Vision AI v3.8.0 (POST /chat/send).');
            if (sendBtn) sendBtn.disabled = false;
            if (stopBtn) stopBtn.hidden = true;
            return;
        }
...
(async function setupAdminButtons() {
    ...
    console.log('👁️ Vision AI v3.8.0 - Ready');
});
