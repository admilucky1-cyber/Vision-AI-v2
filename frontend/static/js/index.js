...
function addMessage(role, text, images = null, modelUsed = null, ragCount = 0, searchUsed = false, imageCount = 0) {
    ...
    if (role === 'ai' && typeof text === 'string') {
        ...
        let src = text;
        // Normalize math fences so KaTeX can render them (```math ... ``` → $$ ... $$)
        src = src
...
async function forceMobileDownload(url, suggestedName) {
    ...
    // Resolve relative paths; force ?dl=1 so server returns octet-stream (mobile save)
    var absolute = url;
    try {
    ...
    if (typeof showToast === 'function') showToast('Preparing download…', 'info', 2000);
    ...
    // Always octet-stream so mobile browsers save instead of inline-play
