...
async function sendMessage() {
    ...
    try {
        ...
        const isImage = /\b(draw|image|picture|photo|generate|create an? |diagram|logo|illustration|photorealistic|architecture)\b/i.test(userMessage || '');
        // Light chat 45s · images 3m · heavy 5m (was 2m/5m/10m — felt stuck on dead providers)
        const timeoutMs = isHeavy ? 300000 : (isImage ? 180000 : 45000);
...
async function startVoiceInput() {
    ...
    if (!Rec) {
        showToast('Voice input needs Chrome, Edge, or Safari (not all in-app browsers).', 'error', 5000);
        return;
    }

    // Re-check permission every click — user may have allowed after a prior deny
    try {
        if (navigator.permissions && navigator.permissions.query) {
            ...
            if (st.state === 'granted') {
                ...
                _micDenied = true;
                _micToast('Microphone blocked. Address bar lock icon → Site settings → Microphone → Allow → Reload.', 'error', 10000);
                return;
            } else {
                _micDenied = false; // prompt
            }
        }
    } catch (_) {
        _micDenied = false;
    ...
    if (_micDenied) {
        _micToast('Microphone still blocked. Address bar lock → Microphone → Allow → Reload this page.', 'error', 10000);
        return;
    }
