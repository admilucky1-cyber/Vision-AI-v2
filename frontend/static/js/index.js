            ...
            try {
                ...
                if (sRes.ok && sRes.body) {
                    ...
                    if (acc) {
                        ...
                        saveHistory();
                        showToast('Streamed from ' + modelUsed, 'success', 2500);
                        return;
                    }
                }
            } catch (streamErr) {
...
(async function setupAdminButtons() {
    ...
    console.log('👁️ Vision AI v4.2.0 - Ready');
});
...
window.addEventListener('resize', function () {
  if (typeof window.forceDesktopSidebarCleanup === 'function') window.forceDesktopSidebarCleanup();
}, { passive: true });
