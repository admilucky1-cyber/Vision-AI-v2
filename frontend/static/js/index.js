...
// ============================================================
// VISION AI v3.8.0 - INDEX LOGIC (Production Ready)
// ============================================================
...
function applyTheme(theme) {
    ...
    if (root.getAttribute('data-theme') === resolved && root.style.colorScheme === (resolved === 'light' ? 'light' : 'dark')) {
...
function isDesktopViewport() {
    try { return window.matchMedia('(min-width: 769px)').matches; } catch (e) { return window.innerWidth > 768; }
}
...
window.toggleSidebar = function() {
    ...
    // Desktop: pure collapse — never touch overlay / body.sidebar-open
    if (!isMobile) {
        document.body.classList.remove('sidebar-open');
        var ov = document.getElementById('sidebar-overlay');
        if (ov) {
        ...
        var collapsing = !sidebar.classList.contains('collapsed-true');
        sidebar.classList.toggle('rail-mode', collapsing);
        sidebar.classList.toggle('collapsed-true', collapsing);
        document.documentElement.setAttribute('data-sidebar', collapsing ? 'closed' : 'open');
    ...
    document.documentElement.setAttribute('data-sidebar', willOpen ? 'open' : 'closed');
};
...
function renderUserProfile() {
    ...
    footer.innerHTML = `
        <div class="user-profile-container" id="profileButton" onclick="window.toggleProfileDropdown(event)" role="button" tabindex="0" aria-haspopup="true" aria-label="Open profile menu">
...
window.performLogout = function() {
    ...
    try {
        ...
        try {
            localStorage.removeItem('vision_ai_is_guest');
            // Do NOT wipe chat history keys — user can sign back in
...
window.forceDesktopSidebarCleanup = function () {
  ...
  var sb = document.getElementById('sidebar');
  if (sb) sb.classList.remove('open-mobile');
  // Ensure header hamburger is never stuck disabled
  document.querySelectorAll('.header-btn, .icon-btn, [onclick*="toggleSidebar"]').forEach(function (el) {
    try {
...
window.addEventListener('resize', function () { if (typeof window.forceDesktopSidebarCleanup === 'function') window.forceDesktopSidebarCleanup(); }, { passive: true });


// v4.2: single debounced resize controller for sidebar reconciliation
(function visionResizeController(){
  var t = null;
  function reconcile(){
    try {
      if (window.innerWidth > 768) {
  ...
  window.addEventListener('resize', function(){
    clearTimeout(t);
    t = setTimeout(reconcile, 150);
  });
  window.addEventListener('orientationchange', function(){ setTimeout(reconcile, 200); });
})();
