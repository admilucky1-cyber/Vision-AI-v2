...
async function sendMessage() {
    ...
    try {
        ...
        const isImage = /\b(draw|image|picture|photo|generate|create an? |diagram|logo|illustration)\b/i.test(userMessage || '');
        const timeoutMs = isHeavy ? 600000 : (isImage ? 300000 : 120000);
...
/* ==== Vision AI 5.1 bridge: prefer modular sidebar/auth ==== */
(function () {
  if (window.__vaSidebar) {
    window.toggleSidebar = window.__vaSidebar.toggleSidebar;
    window.openSidebar = window.__vaSidebar.openSidebar;
    window.closeSidebar = window.__vaSidebar.closeSidebar;
    window.closeMobileSidebar = window.__vaSidebar.closeSidebar;
  }
  if (window.VisionAuth && window.VisionAuth.logout) {
    window.performLogout = window.VisionAuth.logout;
  }
})();
