...
function setUserAvatarGender(g) {
  ...
  if (typeof showToast === 'function') showToast('Avatar: ' + g, 'info', 1200);
}
function avatarEmoji(role) {
  if (role === 'ai' || role === 'assistant' || role === 'system') return '✦';
  var g = getUserAvatarGender();
  if (g === 'male') return '♂';
  if (g === 'female') return '♀';
  return '◉';
}
...
window.toggleSidebar = function() {
    try {
      var sb = document.getElementById('sidebar');
      setTimeout(function() {
        var open = sb && (sb.classList.contains('open-mobile') || window.innerWidth > 768);
    ...
    if (isMobile) {
        ...
        if (!overlay) {
            ...
            overlay.onclick = function() { if (typeof closeMobileSidebar === 'function') closeMobileSidebar(); };
...
function startNewChat() {
    ...
    saveHistory();
    renderHistory();
    const output = document.getElementById('chatOutput') || document.getElementById('output');
...
async function sendMessage() {
    ...
    const isHeavy = /download|transcript|youtube\.com|youtu\.be|mp3|1080p|720p/i.test(userMessage || '') || filesToUpload.length > 0;
    ...
    try {
        updateLoadingStatus(isImage ? "Generating image (GPU may take 1–2 min)…" : "Contacting AI model...");
...
function renderUserProfile() {
    ...
    footer.innerHTML = `
        ...
        <div id="profileDropdown" class="profile-dropdown" style="display:none;" role="menu">
            <div class="avatar-gender-bar" role="group" aria-label="Avatar">
              <button type="button" data-gender="male" onclick="setUserAvatarGender('male')">Male</button>
              <button type="button" data-gender="female" onclick="setUserAvatarGender('female')">Female</button>
              <button type="button" data-gender="neutral" onclick="setUserAvatarGender('neutral')">Neutral</button>
            </div>
...
const PROMPT_LIBRARY = {
    "Study & Exams": {
        icon: "📚", color: "#34d399", tags: ["exam","physics","math","urdu"],
        prompts: [
            { title: "IGCSE / A-Level Physics solver", t: "You are a Cambridge Physics examiner. Solve the uploaded paper or question step-by-step: formula → substitution → numerical answer with units. Flag missing diagrams. Language: [English or Urdu].", tags: ["exam","physics"] },
            { title: "Master concept deep-dive", t: "Explain [concept] at Master's physics level: definitions, assumptions, derivations, limits of validity, and one worked numerical example.", tags: ["explain","physics"] },
            { title: "Guided problem (pause each step)", t: "Solve step-by-step and PAUSE after each step so I can attempt the next. Problem:\n[paste]", tags: ["exam"] },
            { title: "One-page revision sheet", t: "Make a one-page revision sheet for [topic]: core formulas, sign conventions, common mistakes, 5 rapid-fire Qs with answers.", tags: ["exam"] },
            { title: "Compare two theories", t: "Compare [Theory A] vs [Theory B]: assumptions, equations, domain of use, and when each fails.", tags: ["explain"] },
            { title: "Board exam Urdu/English mix", t: "اس سوال کو بورڈ پیپر انداز میں حل کریں۔ فارمولہ، قدم، اور آخری جواب واضح لکھیں۔\n[سوال]", tags: ["urdu","exam"] }
        ]
    },
    "YouTube & Media": {
        icon: "🎥", color: "#f472b6", tags: ["youtube","summary","quiz"],
        prompts: [
            { title: "YouTube summary + timestamps", t: "Summarize this YouTube video with timestamps, key claims, and action items:\n[URL]", tags: ["youtube","summary"] },
            { title: "Urdu video summary", t: "اس یوٹیوب ویڈیو کا اردو میں خلاصہ لکھیں: اہم نکات، نتیجہ، اور تین سبق۔\n[URL]", tags: ["youtube","urdu"] },
            { title: "10-question quiz from video", t: "From the video transcript only, make 10 MCQs with answers and short explanations.\n[URL]", tags: ["youtube","quiz"] },
            { title: "Lesson plan from video", t: "Turn this video into a 40-minute class plan: objectives, activities, assessment.\n[URL]", tags: ["youtube"] }
        ]
    },
    "Coding": {
        icon: "💻", color: "#60a5fa", tags: ["code","debug","api"],
        prompts: [
            { title: "Production bug hunt", t: "Debug this error for a FastAPI/Railway app. Give root cause + minimal patch.\n\nError:\n[paste]\n\nCode:\n[paste]", tags: ["debug"] },
            { title: "Clean refactor", t: "Refactor this code for readability and safety without changing behavior. List changes.\n\n[paste code]", tags: ["code"] },
            { title: "API design", t: "Design REST endpoints for [feature]: paths, request/response JSON, auth, and error codes.", tags: ["api"] },
            { title: "Test cases", t: "Write pytest cases for this function including edge cases:\n[paste]", tags: ["code"] }
        ]
    },
    "Data & BI": {
        icon: "📊", color: "#a78bfa", tags: ["analytics","bi"],
        prompts: [
            { title: "Dataset insights", t: "Propose cleaning steps, 5 charts, and 5 insights for:\n[describe data or paste sample]", tags: ["analytics"] },
            { title: "KPI dashboard plan", t: "Design KPIs and a simple dashboard layout for [business]. Include formulas.", tags: ["bi"] }
        ]
    },
    "Writing": {
        icon: "✍️", color: "#fbbf24", tags: ["email","urdu","rewrite"],
        prompts: [
            { title: "Professional email", t: "Draft email about [topic] for [audience]: subject, body, clear CTA. Tone: [formal/friendly].", tags: ["email"] },
            { title: "Urdu polish", t: "اس متن کو درست اور روان پاکستانی اردو میں لکھیں:\n\n[متن]", tags: ["urdu"] },
            { title: "Executive summary", t: "Turn this into 5 bullets + 2-sentence executive summary:\n\n[paste]", tags: ["rewrite"] }
        ]
    },
    "Career": {
        icon: "🚀", color: "#38bdf8", tags: ["cv","interview","plan"],
        prompts: [
            { title: "Physics → tech roadmap", t: "I have a Master's in Physics (age ~30+). Build a realistic 6-month plan toward [data science / software / quant]: weekly milestones, free resources, portfolio projects.", tags: ["plan"] },
            { title: "CV bullet upgrade", t: "Rewrite this CV bullet with action + metric + result:\n[paste]", tags: ["cv"] },
            { title: "Interview pack", t: "8 interview Q&As for [role] tailored to a physics graduate transitioning to tech.", tags: ["interview"] }
        ]
    },
    "Image & PDF": {
        icon: "🖼️", color: "#fb7185", tags: ["image","pdf"],
        prompts: [
            { title: "Deep image Q&A", t: "Describe every visible detail in the uploaded image (text, UI, labels, diagram parts). Then answer: [question]", tags: ["image"] },
            { title: "Exam PDF solver", t: "Solve the uploaded exam PDF question-by-question with formulas and final answers. Say when a figure is required but missing.", tags: ["pdf","exam"] },
            { title: "Notes from PDF", t: "Extract the document into structured revision notes with headings and key formulas.", tags: ["pdf"] },
            { title: "SDXL image prompt craft", t: "Write a strong image-generation prompt for: [subject]. Include style, lighting, composition, and negative prompt. Keep under 80 words.", tags: ["image"] }
        ]
    }
};
...
function toggleHelpModal() {
    ...
    if (!modal) {
        createHelpModal();
        const _hm = document.getElementById('helpModal');
        _hm.classList.add('ps-drawer-root', 'ps-open');
        _hm.style.display = 'flex';
...
function applyThemePreset(name) {
  ...
  document.documentElement.setAttribute('data-theme-preset', n === 'default' ? '' : n);
...
function initThemePicker() {
  ...
  root.querySelectorAll(".theme-swatch").forEach(function(sw) {
    sw.addEventListener("click", function(e) {
      ...
      } else if (["humanly","nord","ocean","forest","violet","rose","midnight","sunset","default","high-contrast"].indexOf(preset) >= 0) {
        try { localStorage.setItem("vision_ai_theme", "dark"); } catch (err) {}
        if (typeof applyTheme === "function") applyTheme("dark");
      }
