...
function addMessage(role, text, images = null, modelUsed = null, ragCount = 0, searchUsed = false, imageCount = 0) {
    ...
    if (role === 'ai' && typeof text === 'string') {
        ...
        // mark download links for forced download BEFORE injecting HTML
        cleanHtml = cleanHtml.replace(
            /href="([^"]*\/upload\/downloads\/[^"]+)"/g,
            'href="$1" download target="_blank" rel="noopener" class="dl-file-link" data-force-download="1"'
        );
...
const PROMPT_LIBRARY = {
    ...
    "Physics / Exam": {
        ...
        prompts: [
            ...
            { title: "Derive + numeric", t: "Derive the working formula from first principles, then compute with: [data]. Show units at every step.", tags: ["physics"] }
        ]
    },
    "Urdu": {
        icon: "🇵🇰", color: "#a78bfa", tags: ["urdu"],
        prompts: [
            { title: "Accurate Urdu translation", t: "Translate into clear Pakistani Urdu (not overly literary). Keep technical terms in English in parentheses when needed:\n\n[text]", tags: ["urdu"] },
            { title: "Urdu study notes", t: "اس موضوع پر مختصر اردو نوٹس لکھیں: تعریف، فارمولے، اور تین مثالیں۔ موضوع: [topic]", tags: ["urdu"] }
        ]
    },
    "YouTube": {
    ...
    "Creative": {
        icon: "✨", color: "#fbbf24", tags: ["writing"],
        prompts: [
...
async function forceMobileDownload(url, suggestedName) {
    ...
    var absolute = url;
    try { absolute = new URL(url, window.location.origin).href; } catch (e) {}
    var token =
    ...
    // Prefer octet-stream so mobile browsers treat as download, not inline media
    if (blob.type && /^(video|audio)\//i.test(blob.type)) {
    ...
    if (!name || name === 'true' || name === 'download') {
        try {
            ...
            if (m) name = decodeURIComponent((m[1] || m[2] || m[3] || '').replace(/"/g, '').trim());
        } catch (e) {}
    }
    if (!name) name = (absolute.split('/').pop() || 'download.bin').split('?')[0];

    // iOS Safari: share sheet if available and blob download is flaky
    ...
    if (isIOS && navigator.share && navigator.canShare) {
        try {
            ...
            if (navigator.canShare({ files: [file] })) {
                ...
                if (typeof showToast === 'function') showToast('Share sheet opened — save the file', 'success', 3500);
                return;
            }
        } catch (shareErr) {
    ...
    // Synchronous user-gesture-friendly click
    a.click();
    setTimeout(function () {
...
function bindDownloadLinks(root) {
    ...
    nodes.forEach(function (el) {
        ...
        el.addEventListener('click', function (ev) {
            ...
            forceMobileDownload(href, suggested).catch(function (err) {
