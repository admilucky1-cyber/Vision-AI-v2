...
/* v2.7.3: public-ready sidebar — no stuck dim overlay */
  ...
  function clearDim() {
    try {
      ...
      if (sb && window.innerWidth > 768) {
        ...
        // Keep sidebar visible by default (remove invisible rail)
        sb.classList.remove('rail-mode');
      }
    } catch (e) {}
  }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', clearDim);
  } else {
    clearDim();
  }
  window.addEventListener('resize', clearDim);
  window.addEventListener('load', clearDim);
})();


// Ensure overlay never sticks on load (mobile dark veil bug)
...
window.toggleSidebar = function() {
    ...
    if (isMobile) {
        ...
        document.body.classList.toggle('sidebar-open', sidebar.classList.contains('open-mobile'));
...
async function sendMessage() {
    ...
    const isImage = /\b(draw|image|picture|photo|generate|create|diagram|logo|illustration|photorealistic|architecture|mosque|masjid|building)\b/i.test(userMessage || '');
    try {
        ...
        const timeoutMs = isHeavy ? 300000 : (isImage ? 300000 : 45000);
...
function downloadCurrentChat() {
  try {
    const chat = (typeof chatHistory !== 'undefined' && currentChatId)
      ? chatHistory.find(c => c.id === currentChatId) : null;
    const title = (chat && chat.title) || 'vision-ai-chat';
    let body = '# ' + title + '\n\n';
    const msgs = (chat && chat.messages) || [];
    if (!msgs.length) {
      const out = document.getElementById('chatOutput');
      body += (out && out.innerText) ? out.innerText : '(empty chat)';
    } else {
      msgs.forEach(m => {
        body += '## ' + (m.role || 'msg') + '\n' + (m.text || '') + '\n\n';
      });
    ...
    if (typeof showToast === 'function') showToast('Chat downloaded', 'success', 2000);
  } catch (e) {
