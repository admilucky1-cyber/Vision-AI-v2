...
window.toggleSidebar = function() {
    ...
    if (!sidebar) return;
    const isMobile = isDrawerViewport();
...
window.performLogout = async function() {
    if (window.__vaLoggingOut) return;
    if (!confirm('Log out?')) return;
    window.__vaLoggingOut = true;
    var btn = document.querySelector('.logout-item, #profileDropdown .logout-item');
