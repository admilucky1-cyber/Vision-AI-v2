...
function visionImageToolbar(img) {
  ...
  let h = "";
  if (src) {
    h += '<a class="img-act" href="' + src + '" download="vision-image.png">⬇️ Download</a>';
    h += '<button type="button" class="img-act" onclick="window.open(this.previousElementSibling.href,\'_blank')">⛶ Full</button>';
  }
  if (img && img.svg_data) {
...
function initThemePreset() {
  try {
    const n = localStorage.getItem('vision_theme_preset') || 'default';
    applyThemePreset(n);
  } catch (e) {}
}
