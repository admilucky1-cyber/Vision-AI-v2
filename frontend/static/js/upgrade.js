// ============================================================
// VISION AI v2.0 - UPGRADE LOGIC (Production Ready)
// ============================================================
...
function formatPrice(plan) {
    ...
    if (plan.price === 0) {
        return `<div class="plan-price">Free</div>`;
    }
    
    return `<div class="plan-price"><sup>$</sup>${plan.price.toFixed(2)}<span class="per"> / ${escapeHtml(plan.billing || 'month')}</span></div>`;
}
    ...
    try {
        ...
        const methodsHtml = info.methods.map(m => {
            if (m.id === 'easypaisa') {
                ...
                return `
                    <div style="padding:12px 0;border-bottom:1px solid var(--border-color, rgba(0,0,0,0.08));display:flex;gap:14px;align-items:center;flex-wrap:wrap;">
                        <div style="flex:1;min-width:160px;">
                            <strong>${escapeHtml(m.name)}</strong><br>
                            <span style="font-size:1.05rem;letter-spacing:0.02em;">${escapeHtml(m.number)}</span>
                        ...
                        ${payload ? `<img src="${qrUrl(payload)}" alt="Easypaisa QR" width="160" height="160" style="border-radius:12px;background:#fff;padding:6px;" loading="lazy">` : ''}
                    </div>`;
            }
            const iban = m.iban || '';
            return `
                <div style="padding:12px 0;border-bottom:1px solid var(--border-color, rgba(0,0,0,0.08));display:flex;gap:14px;align-items:center;flex-wrap:wrap;">
                    <div style="flex:1;min-width:160px;">
                        <strong>${escapeHtml(m.name)}</strong><br>
                        IBAN: <span style="word-break:break-all;">${escapeHtml(iban)}</span>
                        ${m.title ? `<br><span style="color:var(--text-muted);font-size:0.85rem;">Account title: ${escapeHtml(m.title)}</span>` : ''}
                        <br><span style="color:var(--text-muted);font-size:0.8rem;">Scan QR for bank transfer details</span>
                    </div>
                    ${iban ? `<img src="${qrUrl(iban)}" alt="IBAN QR" width="160" height="160" style="border-radius:12px;background:#fff;padding:6px;" loading="lazy">` : ''}
                </div>`;
        }).join('');
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    console.log('👁️ Vision AI Upgrade v2.0 - Ready');
});
