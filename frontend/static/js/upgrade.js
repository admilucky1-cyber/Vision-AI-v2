...
async function loadPaymentMethods() {
    ...
    try {
        ...
        const methodsHtml = info.methods.map(m => {
            if (m.id === 'easypaisa') {
                return `
                    <div style="padding:10px 0;border-bottom:1px solid var(--border-color, rgba(0,0,0,0.08));">
                        <strong>${escapeHtml(m.name)}</strong>: ${escapeHtml(m.number)}
...
async function submitPayment(event) {
    ...
    try {
        ...
        if (res.ok) {
            if (msgEl) {
                msgEl.textContent = `✅ ${data.message || 'Payment submitted for verification.'}`;
                msgEl.style.color = '#4ade80';
            }
            showStatus('Payment submitted! We\'ll activate your plan after verification.', 'success');
            event.target.reset();
        } else {
