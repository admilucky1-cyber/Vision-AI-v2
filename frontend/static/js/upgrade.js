...
function formatPrice(plan) {
    if (!plan) return '';
    if (plan.price === 0 || plan.price === 0.0) {
        return `<div class="plan-price">Free</div>`;
    }
    const cur = String(plan.currency || 'PKR').toUpperCase();
    const amount = Number(plan.price);
    if (cur === 'PKR' || cur === 'RS') {
        // Pakistan: show Rs 799 / month (no fake dollar sign)
        const n = Math.round(amount).toLocaleString('en-PK');
        return `<div class="plan-price">Rs ${n}<span class="per"> / ${escapeHtml(plan.billing || 'month')}</span></div>`;
    }
    if (cur === 'USD') {
        return `<div class="plan-price"><sup>$</sup>${amount.toFixed(2)}<span class="per"> / ${escapeHtml(plan.billing || 'month')}</span></div>`;
    }
    return `<div class="plan-price">${escapeHtml(cur)} ${amount.toFixed(2)}<span class="per"> / ${escapeHtml(plan.billing || 'month')}</span></div>`;
}
