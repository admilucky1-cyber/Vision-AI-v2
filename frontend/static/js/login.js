...
// Show Google OAuth errors returned as ?error=
document.addEventListener('DOMContentLoaded', function() {
    try {
        const p = new URLSearchParams(window.location.search);
        const err = p.get('error');
        if (err) {
            const msg = decodeURIComponent(err).replace(/_/g, ' ');
            if (typeof showError === 'function') showError('Google sign-in: ' + msg);
            else if (typeof showAuthToast === 'function') showAuthToast('Google sign-in: ' + msg, 'error');
