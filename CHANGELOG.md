...
- Premium Theme button + topbar chip styling
- Welcome empty state; remove duplicate free-tier line in input

## 3.0.2 — 2026-08-11

- Mobile YouTube/file download via blob + share sheet (iOS)
- Prompt Studio marketplace drawer + fresh prompt library
- Settings admin panels: payments + search cache
- Plans glow, banner/tagline/sidebar polish

## 3.0.1b — Railway deploy harden
- Soft-import skills/agent so boot cannot die on optional routers
- Align HTML app-version meta to 3.0.1
- RAILWAY_DEPLOY.md troubleshooting

## 3.0.1 — 2026-08-11

- Fix login/register `[object Object]` (FastAPI validation detail normalization)
- Working Continue as guest via `POST /auth/guest`
- Login card animation + header/tagline spacing
- AgentOrchestrator Colab keep-alive every 5 minutes
- Skill suggestions API + enhanced API Vault auto-retry/health

## 3.0.0 — 2026-08-11

- Full glassmorphism UI (`unified-v300.css`) with CSS variables + 0.35s transitions
- Themes: +Emerald, +Frost, +Ember (15 total)
- `services/api_vault.py` — multi-provider keys + fallback + connection test
- `SmartImageRouter` — Colab → HF → Pollinations
- Skill Market (`skills.html` + `skill_router.py` + `/api/skills`)
- Glass intensity mode (off / soft / strong) via `toggleGlassMode()`

## 2.9.2 — 2026-08-10

- Sidebar + topbar follow every theme preset (shell CSS variables)
- Clearer 3D logo (crisper, less extreme tilt)
- Sample zips reviewed: not merged; yt-dlp stays official PyPI
- SAMPLES_REVIEW.md

## 2.9.1 — 2026-08-10

- 3D logo treatment (perspective + glow)
- Premium animated orbs for AI/user avatars (male/female/neutral)
- Profile menu layout fixed (3-col gender, no white bar, clean links)
- Gender choice updates live message avatars

## 2.9.0 — 2026-08-10

- Adaptive text colors so chat never goes invisible across themes
- Theme menu z-index above chat; more presets (Ocean, Forest, Violet, Rose, Midnight, Sand)
- Animated AI/user avatars; Male/Female/Neutral choice in profile menu
- Plan feature text contrast on dark cards
- Header brand no longer fully disappears with sidebar open

## 2.8.9 — 2026-08-10

- Plans: force full-width grid; payment shell balanced
- Chat: remove redundant hint chips; improve bubble/composer readability
- Header: hide brand when sidebar open; quieter Stop button
- Profile menu: Settings, Upgrade, Download chat, Log out
- README updated to v2.8.9

## 2.8.8 — 2026-08-10

- Plans: sort lowest→highest price; medium card height; 5-col wide grid
- Payment section compact + mobile single-column form/QR
- Theme presets work on upgrade page (dark + light)
- Responsive breakpoints for plan cards

## 2.8.7 — 2026-08-10

- Login page redesigned (Humanly Teal glass, Login/Register, Google, guest)
- Sidebar profile menu restyled (Settings / Upgrade / Log out)
- Download chat + profile layout fixed in footer
