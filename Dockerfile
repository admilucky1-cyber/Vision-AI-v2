...
# Default 1 worker until state is moved to Postgres/Redis (JSON + multi-worker is unsafe)
CMD ["sh", "-c", "uvicorn main:app --host 0.0.0.0 --port ${PORT:-5050} --workers ${WEB_WORKERS:-1} --proxy-headers --forwarded-allow-ips=*"]
