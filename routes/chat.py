...
async def chat_send(
    ...
    try:
        ...
        # v2.7.0: re-rank long document context vs the user question
        try:
            if extra_context and len(extra_context) > 2500 and (message or "").strip():
                ...
                logger.info(f"RAG re-rank applied → context {len(extra_context)} chars")
        ...
        if pure_image_request and generate_images:
            ...
            try:
                ...
                if images:
                    ...
                    answer = (
                        ...
                        "4. Or set `HF_TOKEN` for serverless fallback"
                    )
            except Exception as e:
            ...
            try:
                ...
                # Longer talks need more than 32s (provider cascade + search)
                _budget = 120.0 if _heavy else 75.0
                ...
                answer = (
                    "The AI took too long on this turn (network or free-tier load).\n\n"
                    "**Try:** select **Groq** in the model menu (faster), shorten the message, "
                    "or send again in a few seconds."
                )
            except Exception as e:
