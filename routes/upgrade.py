...
class PlanConfig:
    ...
    PLANS: Dict[str, dict] = {
        ...
        "pro": {
            ...
            "stripe_price_id": "price_pro_monthly",  # Placeholder for Stripe
        },
        "team": {
            "id": "team",
            "name": "Team",
...
async def submit_payment_request(
    ...
    cfg = _payment_config()
    expected = cfg["pro_price_pkr"] if body.plan == "pro" else cfg.get(f"{body.plan}_price_pkr", cfg["pro_price_pkr"])
    if body.plan == "team":
    ...
    row = {
        ...
        "amount_pkr": body.amount_pkr or expected,
    ...
    return {
        "status": "pending",
        "message": "Payment submitted. Admin will verify and activate your plan (usually within a few hours).",
        "request_id": row["id"],
        "expected_pkr": expected,
    }
